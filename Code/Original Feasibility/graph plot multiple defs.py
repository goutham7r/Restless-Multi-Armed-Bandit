...
def graph_approx(T_max, f_all):
	...
	for T in range(1,T_max+1):
		...
		if T>9:
			...
			s1+=approx[1]
			c+=1
		data0.append(approx[0])
		data1.append(approx[1])
	ar0=[]
	avg0=[]
	ar1=[]
	avg1=[]
	...
	for i in range(T_max):
		...
		ar1.append(worst[1])
		avg1.append(s1)
	plt.plot(X,data0,X,data1,X,ar0,X,avg0,X,ar1,X,avg1)
	plt.axis([1, T_max, 0, 101])
	plt.title('Approximation Ratio vs Length of sequence')
	plt.text((T_max/2), worst[0]-10,'Worst Def 1= '+"{:.2f}".format(worst[0]) + " percent at T=" + str(worst_index[0]))
	plt.text((T_max/8), worst[0]-20,"Average Def 1 approximation ratio from T=10 to T=" + str(T_max) +" is "+"{:.2f}".format(s0)+" percent")
	plt.text((T_max/2), worst[1]-10,'Worst Def 2= '+"{:.2f}".format(worst[1]) + " percent at T=" + str(worst_index[1]))
	plt.text((T_max/8), worst[1]-20,"Average Def 2 approximation ratio from T=10 to T=" + str(T_max) +" is "+"{:.2f}".format(s1)+" percent")
	plt.xlabel('T')
