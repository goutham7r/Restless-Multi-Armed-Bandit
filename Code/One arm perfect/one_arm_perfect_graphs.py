...
class set_of_functions:
	...
	def Optimum_Sequence(self,T):
		...
		for func in self.f:
			if func.label>0:
				list_of_best.append([func.label]+func.dynamic(T))
		list_of_best.sort(key=lambda x:x[3])
		list_of_best.sort(key=lambda x:x[2], reverse=True)
		for i in list_of_best:
			print i
		seq=list_of_best[0][1]
		rew=list_of_best[0][2]
		print "Sequence: ",seq,"Reward: ",rew
		opt_seq,max_rew=self.dynamic(T)
		print "Optimum Sequence: ",opt_seq,"Maximum Reward: ",max_rew
		approx = 100.0*rew/max_rew
